  		...
  		if(in_array($fileActualExt, $allowed))
  			{
  				if($fileError === 0)
  				{
                    ...
                    //to upload gambar and change the name so that bila user masuk nama sama dia tak clash
                        ...
                        $query = "INSERT INTO topping(adminID, toppingName, toppingPrice, toppingImg)
                        VALUES ('".$adminID."','".$toppingName."', '".$toppingPrice."', 'assets/imgs/".$fileName."')";


         			if ($link->query($query))
               			{
               				echo ("<SCRIPT LANGUAGE='JavaScript'>
               				window.alert('New topping is added!')
                            ...
                            <form method="POST" id="form" enctype="multipart/form-data"> 
                                <div class="form-field">
                                    <label>Topping Name</label>
                                        <input type="text" id="toppingName" name="toppingName" class="form-control" required><br>
                                </div>
                                <div class="form-field">
                                    <label>Topping Price (RM)</label>
                                        <input type="text" onkeypress="return isNumberKey(event)" id="toppingPrice" name="toppingPrice" class="form-control" required><br>
                                </div>
                                <div class="form-field">
